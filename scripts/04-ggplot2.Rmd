...
imdb %>% 
  mutate(cor_graf = "blue") %>% 
  ggplot() +
  geom_point(aes(x = orcamento, y = nota_imdb, color = cor_graf)) +
...
imdb %>%
  ...
  facet_wrap(ano_cat ~ classificacao)
```
...
imdb %>% 
  ...
  ungroup() %>% 
  mutate(
    diretor = fct_reorder(diretor, lucro, .fun = min, na.rm = TRUE)
  ) %>% 
  ggplot() +
...
imdb %>% 
  ...
  ggplot(aes(x = diretor, y = n, label = n)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) + geom_label(
    position = position_stack(vjust = 1.1), #position_stack(vjust = .5)
    colour = "yellow",
    fill = "purple",
    fontface = "bold",
    label.size = 0
...
imdb %>% 
    ...
    group_by(diretor) %>% 
    mutate(
      numero_de_filmes_na_base_do_diretor = n()
    ) %>% 
    ungroup() %>% 
    filter(numero_de_filmes_na_base_do_diretor >= 15)
```
...
imdb %>% 
  filter(str_detect(generos, "Animation"),
         !is.na(diretor)) %>%
  count(diretor) %>% 
  top_n(10, n) %>% 
...
imdb %>% 
  ...
  filter(n() > 15) %>% 
  ggplot() +
  geom_histogram(mapping = aes(x = lucro)) +
  facet_wrap(~diretor)
...
imdb %>% 
  ...
  geom_bar(alpha = 0.1, stat = 'identity') +
  geom_line() +
  geom_point() +
  geom_text()
```
...
imdb %>% 
  filter(ator_1 %in% a_c_m_f$ator_1) %>% 
  ggplot(aes(x = orcamento)) +
  geom_histogram(binwidth = 100*10^6)

imdb %>% 
  filter(ator_1 %in% a_c_m_f$ator_1) %>% 
  ggplot(aes(x = lucro)) +
  geom_histogram(binwidth = 50*10^6)
...
imdb %>% 
  ...
  filter(numero_de_filmes > 
    sort(unique(variavel$numero_de_filmes),
         decreasing = TRUE)[8])
...
imdb %>%
  ...
  scale_x_continuous(label = scales::dollar_format()) +
  theme_minimal() +
  ggtitle("Arrecadação vs Orçamento (em milhões de dólares)", subtitle = "Filmes de grande orçamento dão bastante prejuízo.") +
...
imdb %>% 
  ...
  top_n(10, receita) %>%
  mutate(titulo = fct_reorder(titulo, receita)) %>% 
  ggplot(aes(x = titulo, y = receita)) +
  geom_col() +
  coord_flip()
